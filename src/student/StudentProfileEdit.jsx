...
import './Style/MyCourses.css'; // per .landing-btn, .glass-card, .glass-hero
...
export default function StudentProfileEdit() {
  ...
  // Stato profilo (drive del form)
  const [profile, setProfile] = useState({
    name: '',
    surname: '',
    role: 'Studente',
    degree: '',     // Corso di studi
    year: '',       // Anno
    matricola: '',  // Matricola (sola lettura se già presente)
    email: '',
    phone: '',
    campus: '',     // Sede
    bio: 'Breve biografia dello studente…',
    website: '',
    linkedin: '',
    github: '',
  });

  // Avatar (preview locale — upload a storage sarà aggiunto in futuro)
  ...
  /** Caricamento iniziale da Auth + Firestore */
  useEffect(() => {
    let alive = true;
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        // Backfill dei campi base se mancanti
        ...
        setProfile((p) => ({
          ...
          name,
          surname,
          role: data?.role === 'teacher' ? 'Docente' : 'Studente',
          degree: data?.degree || '',
          year: data?.year || '',
        ...
        console.error('StudentProfileEdit load', e);
  ...
  const onSaveProfile = async (e) => {
    ...
    try {
      ...
      // La matricola, se presente, non la forziamo a mano: la si considera immutabile lato UI
      if (!profile.matricola) {
        // nulla – potrebbe essere gestita da segreteria o altro flusso
      }
  ...
  if (loading) {
    return (
      <Container className="student-profile-page py-4">
  ...
  return (
    <Container className="student-profile-page py-4">
      <section className="glass-hero text-white mb-4">
        <h1 className="hero-title mb-1">Profilo Studente</h1>
        <p className="hero-subtitle mb-2">
          Gestisci dati personali, contatti e sicurezza dell’account
        </p>
        <div className="d-flex gap-2 flex-wrap justify-content-center mt-2">
          <Badge bg="light" text="dark">{profile.role || 'Studente'}</Badge>
      ...
      <Row className="g-3 align-items-stretch">
        ...
        <Col xs={12} lg={4}>
                ...
                <div className="avatar-ring">
                  {avatarPreview ? (
                    ...
                    <div className="avatar-placeholder">
                      ...
                      {(profile.surname?.[0] || 'T').toUpperCase()}
                    </div>
                  )}
                </div>
              </div>
          ...
          {/* Preferenze (placeholder) */}
          <Card className="glass-card">
            <Card.Body>
              <h5 className="mb-3">Preferenze notifiche</h5>
              <div className="text-white-50">Le preferenze verranno aggiunte più avanti.</div>
            </Card.Body>
          </Card>
        </Col>
              ...
              <Form onSubmit={onSaveProfile}>
                  ...
                  <Col sm={6}>
                    <Form.Group className="mb-3">
                      ...
                      <Form.Control
                        ...
                        onChange={onChange}
                        placeholder="es. 736830"
                        disabled={!!profile.matricola}
                      />
                    </Form.Group>
                  </Col>
                </Row>
                  ...
                  <Col sm={6}>
                    <Form.Group className="mb-3">
                      ...
                      <Form.Control
                        ...
                        value={profile.email}
                        onChange={onChange}
                        placeholder="email@studenti.ateneo.it"
                      />
                      <Form.Text className="text-white-50">
                        L’allineamento con l’email in Auth si fa nella sezione Sicurezza.
                      </Form.Text>
                    </Form.Group>
                  </Col>
                  <Col sm={6}>
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Control
                    ...
                    value={profile.bio}
                    onChange={onChange}
                    placeholder="Studente appassionato di tecnologie e apprendimento digitale…"
                  />
                </Form.Group>
                ...
                <Row className="g-2">
                  <Col md={6}>
                    <Form.Group className="mb-3">
                      ...
                      <Form.Control
                        type="email"
                        placeholder="nuova.email@studenti.ateneo.it"
