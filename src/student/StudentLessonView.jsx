// src/student/StudentLessonView.jsx
import React, { useEffect, useRef, useState } from 'react';
import { useParams } from 'react-router-dom';
import './Style/StudentLessonView.css';

import ChatBox from '../components/ChatBox';
import { getCourseById, listSections, listLessons } from '../services/courses';

export default function StudentLessonView() {
  // Teacher usa :courseId/:secId/:lezId. La rotta studente usa :id/:secId/:lezId.
  // Supporto entrambi per sicurezza.
  const {
  ...
  const courseId = courseIdParam || idParam;    // <-- adattamento rispetto al Teacher
  ...
  // Misuro l'altezza del media per pareggiare la chat SOLO su desktop
  ...
  // Rimisuro quando cambia la view o la lezione caricata
  useEffect(() => { measure(); }, [view, lesson]);

  // === LOGICA IDENTICA AL TEACHER ===
  useEffect(() => {
    let alive = true;
    (async () => {
      try {
        ...
        // Guard: se manca qualche ID evito di chiamare i service (come fallback "not found")
        if (!courseId || !secId || !lezId) {
          setCourseOK(!!courseId);
          setSectionOK(!!secId);
          setLesson(null);
          return;
        }
  ...
  // Stessa regola del Teacher: chat pareggiata a desktop, fissa in responsive
  const chatHeightStyle = isDesktop ? { height: mediaH } : { height: 'min(60vh, 520px)' };
  ...
  if (hasVideo && !hasPdf) {
    return (
      <div className="lesson-video-page">
        ...
        <div className='description-div-singoli'>
